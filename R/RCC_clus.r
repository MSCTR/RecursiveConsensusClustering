...
CDF_RCC = function(zscore_mat,output_dir,maxK,times,rIN, selected_mat, minGenesP, threshold){
	...
	recursive_consensus = function(sample_ids,col_num,threshold,output_dir){
		...
		if((nrow(sample_ids) < min_samples)){
			...
			cluster_parallel <<- pforeach(times = 1:iterations, .combine = append, .parallel = T) ({
				cluster_slopes1 = CDF_RCC(zscore_mat,output_dir,maxK, times, rIN, selected_mat, minGenesP, threshold)
			})
